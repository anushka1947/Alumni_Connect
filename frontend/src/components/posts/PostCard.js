...
export default function PostCard({ post, refreshFeed }) {
  ...
  /* ================================================= */
  return (
    <div className="bg-black border border-white/5 rounded-3xl shadow-[0_4px_30px_rgba(0,0,0,0.5)] overflow-hidden transition-all duration-300 hover:shadow-[0_8px_40px_rgba(0,0,0,0.8)] hover:border-white/10 relative group">
      {/* Subtle Glow on hover */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.03),transparent_50%)] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
      ...
      <div className="p-3 sm:p-4 lg:p-5">
        <div className="flex flex-wrap items-start gap-3 sm:gap-4 mb-3 sm:mb-4">
          ...
          >
            ...
            <div className="ml-2 sm:ml-3 text-left min-w-0 flex-1">
              <p className="font-semibold text-white tracking-tight text-sm sm:text-base truncate">
                {post.user?.fullName}
              </p>
              <div className="flex items-center gap-1 sm:gap-2 text-xs text-muted">
                <span>{formatDistanceToNow(new Date(post.createdAt))} ago</span>
                {post.user.role === "admin" && (
                  <span className="bg-primary/10 text-primary border border-primary/20 px-1.5 sm:px-2 py-0.5 rounded-full text-xs flex-shrink-0">
                    Admin
                  </span>
                )}
              </div>
          ...
          {/* Right-side actions */}
          <div className="ml-auto flex items-center gap-1 sm:gap-2 flex-shrink-0 relative z-10">
            <span className="bg-white/5 border border-white/10 text-white/50 px-3 py-1 rounded-full text-xs font-medium truncate max-w-24">
              {post.category}
            </span>
        ...
        <div className="space-y-3 sm:space-y-4 relative z-10">
          ...
          {/* Content */}
          <div className="text-sm sm:text-base text-white/70 font-light leading-relaxed whitespace-pre-wrap">
            <Linkify
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/5">
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 sm:gap-2 text-sm transition-colors ${isLiked
                  ? "text-primary drop-shadow-[0_0_12px_rgba(147,51,234,0.6)]"
                  : "text-white/40 hover:text-white"
                  }`}
              >
                <HeartIconSolid className={`h-4 w-4 sm:h-5 sm:w-5 ${isLiked ? "fill-current text-primary" : "text-white/40 group-hover:text-white/60"
                  }`} />
              ...
              <button
                ...
                className="flex items-center gap-1.5 sm:gap-2 text-sm text-white/40 hover:text-white transition-colors"
              >
                <ChatBubbleOvalLeftIcon className="h-4 w-4 sm:h-5 sm:w-5 group-hover:text-white/60" />
                <span className="hidden sm:inline">
            ...
            <button
              onClick={handleShare}
              disabled={isSharing}
              className={`flex items-center gap-1.5 sm:gap-2 text-sm text-white/40 hover:text-white transition-colors ${isSharing ? "cursor-not-allowed opacity-50" : ""
                }`}
            >
              <ShareIcon className="h-4 w-4 sm:h-5 sm:w-5 group-hover:text-white/60" />
