...
export default function Navbar() {
  ...
  return (
    <div className="sticky top-0 z-50 pt-4 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full pointer-events-none">
      <Disclosure
        as="nav"
        className="bg-black/40 backdrop-blur-2xl border border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)] rounded-2xl transition-all duration-300 pointer-events-auto"
      >
        {({ open, close }) => (
          <>
            <div className="px-4 sm:px-6 lg:px-8">
              <div className="flex h-16 justify-between items-center">
                ...
                <div className="flex items-center">
                  <Link
                    to="/"
                    className="text-lg sm:text-xl lg:text-2xl font-extrabold text-white flex items-center shrink-0 tracking-tight"
                  >
                    <SparklesIcon className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-primary" />
                    <span className="hidden sm:block">KV Bhopal</span>
                    <span className="sm:hidden">KV Bhopal</span>
                  </Link>
                    ...
                    {navigation.map((item) =>
                      item.onClick ? (
                        <a
                          ...
                          className="text-muted hover:text-white hover:bg-white/5 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 whitespace-nowrap"
                        >
                          {item.name}
                        </a>
                      ) : (
                ...
                <div className="flex items-center space-x-2 sm:space-x-4">
                  {user ? (
                    <>
                      ...
                      >
                        <button
                          ...
                          className="p-2 rounded-full text-muted hover:text-white hover:bg-white/5 transition-colors relative"
                        >
                      ...
                      <Menu as="div" className="hidden sm:block md:hidden relative">
                        ...
                        >
                          <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-xl bg-black border border-white/10 py-1 shadow-2xl focus:outline-none">
                            <Menu.Item>
                              {({ active }) => (
                                <Link
                                  to="/profile"
                                  className={classNames(
                                    active ? "bg-white/5" : "",
                                    "block px-4 py-2 text-sm text-muted hover:text-white transition-colors"
                                  )}
                                >
                                  Your Profile
                                </Link>
                            ...
                            <Menu.Item>
                              {({ active }) => (
                                <button
                                  ...
                                  className={classNames(
                                    active ? "bg-white/5" : "",
                                    "block w-full text-left px-4 py-2 text-sm text-muted hover:text-white transition-colors"
                                  )}
                                >
                                  Sign out
                                </button>
                    ...
                    <>
                      ...
                      <div className="hidden sm:flex items-center space-x-2">
                        <Link
                          to="/login"
                          className="text-sm font-medium text-muted hover:text-white transition-colors px-3 py-2 rounded-md hover:bg-white/5"
                        >
                          Sign In
                        </Link>
                        <Link
                  ...
                  <div className="flex items-center md:hidden">
                    <Disclosure.Button className="inline-flex items-center justify-center rounded-lg bg-white/5 p-2 text-muted hover:bg-white/10 hover:text-white focus:outline-none transition-colors">
                  ...
                  >
                    {item.onClick ? (
                      <a
                        ...
                        className="block rounded-lg py-3 px-3 text-base font-medium text-muted hover:bg-white/5 hover:text-white transition-colors"
                      >
                        {item.name}
                      </a>
                    ) : (
                        ...
                        className={({ isActive }) =>
                          classNames(
                            isActive
                              ...
                              : "text-muted hover:bg-white/5 hover:text-white rounded-lg",
                            "block py-3 px-3 text-base font-medium transition-colors"
                          )
                        }
                      >
                        {item.name}
