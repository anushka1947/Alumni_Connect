...
export default function Register() {
    ...
    const getInputClassName = (fieldName) => {
        const baseClasses = "w-full px-4 py-3 border rounded-xl bg-black/40 text-on-surface placeholder-muted focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all duration-300 text-sm sm:text-base";
        return `${baseClasses} ${errors[fieldName] ? 'border-red-500 focus:ring-red-500' : 'border-white/10 focus:border-transparent'}`;
    };

    return (
        <div className="flex items-center justify-center min-h-[80vh] py-8 sm:py-12 px-3 sm:px-4 lg:px-8 relative overflow-hidden">
            ...
            <div className="max-w-md w-full space-y-6 sm:space-y-8 p-6 sm:p-8 lg:p-10 bg-surface/40 backdrop-blur-xl border border-white/10 rounded-3xl shadow-premium relative z-10">
                <div>
                    <h2 className="mt-4 sm:mt-6 text-center text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-primary-light via-primary to-secondary drop-shadow-sm pb-1">Create Your Account</h2>
                ...
                <form className="mt-6 sm:mt-8 space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div>
                        <button
                            type="submit"
                            disabled={loading}
                            className={`w-full py-3 px-4 rounded-xl text-white text-sm font-medium transition-all duration-300 shadow-premium premium-button overflow-hidden ${loading ? 'bg-surface/50 cursor-not-allowed' : 'bg-gradient-to-r from-primary-light to-primary hover:from-primary hover:to-primary-dark hover:shadow-premium-hover transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-primary/50'
                                }`}
                        >
